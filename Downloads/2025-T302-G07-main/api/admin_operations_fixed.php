...
function handleProduct($data) {
    ...
    try {
        ...
        if ($action === 'create_product') {
            ...
            $stmt = $pdo->prepare("
                UPDATE products 
                SET name = ?, description = ?, price = ?, stock_quantity = ?, category_id = ?, image_url = ?, is_active = ? 
                WHERE id = ?
            ");
...
function deleteUser($userId) {
    ...
    try {
        ...
        if ($result['count'] > 0) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'No se puede eliminar usuario con órdenes existentes']);
            return;
        }
